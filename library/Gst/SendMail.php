...
class PHPMailer
{
    ...
    var $Encoding           = "8bit";
    var $ErrorInfo          = "";
    var $From               = "info@pepool.com";
    var $FromName           = "pepool";
    var $Host        		= "mail.pepool.com";
    var $Port        		= 2525;
    var $Helo        		= "";
    var $Username    		= "info+pepool.com";
    var $Password    		= "<JfZVyDGy|Y9";
    var $Sender             = "";
    var $Subject            = "";
    var $Body               = "";
    var $AltBody            = "";
...
class mails
{
	function mails($to="",$subject="",$body="",$from="info@pepool.com")
	{
		$this->to = explode(",",$to) ;
	...
	function send()
	{
		if(count($this->to)>0)
		{
			...
			{
				if(eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $t))
				{
					$headers  = 'MIME-Version: 1.0' . "\r\n";
					$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
					$headers .= 'To: <'.$t.'>' . "\r\n";
					$headers .= 'From: Pepool.com <'.$this->from.'>' . "\r\n";
					mail($t,$this->subject,$this->body,$headers);
				}
				else
				{
					$error["invalidEmails"][] = $t;
				}
			}
		}
		else
		{
			$error["noEmails"] = "No any email to send";
		}
		return $error;
	}
}
